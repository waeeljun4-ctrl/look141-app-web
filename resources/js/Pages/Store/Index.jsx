        ...
        <div ref={cardRef} onClick={() => onOpen(product)}
            ...
            <div className="h-44 bg-gradient-to-br from-cream-2 to-cream-3 dark:from-ink dark:to-ink-2 flex items-center justify-center text-4xl relative overflow-hidden">
                {(product.image || product.images?.[0])
                    ? <img src={`/storage/${product.image || product.images[0]}`} alt={localField(product, 'name', locale)} className={`w-full h-full object-cover ${soldOut ? 'grayscale opacity-50' : ''}`} />
                    : '👕'
                }
                {soldOut && (
                ...
                <button onClick={e => { e.stopPropagation(); toggleWishlist({ ...product, name: localField(product, 'name', locale) }); }}
                    className={`absolute top-2.5 start-2.5 w-8 h-8 rounded-full flex items-center justify-center text-sm z-10 transition-colors
                        ${inWishlist(product.id) ? 'bg-red-500 text-white' : 'bg-white/80 text-ink hover:bg-white'}`}>
                    {inWishlist(product.id) ? '♥' : '♡'}
                </button>
                {discount && (
                    <span className="absolute top-2.5 end-2.5 bg-red-500 text-white text-xs font-black px-2 py-0.5 rounded-full z-10">-{discount}%</span>
                )}
                {!discount && product.badge && (
                    <span className="absolute top-2.5 end-2.5 bg-accent text-white text-xs font-bold px-2 py-0.5 rounded-full z-10">
                        {product.badge}
                    </span>
                )}
            </div>
                ...
                {sizes.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-2">
                        {sizes.slice(0, 6).map(s => (
                            <span key={s} className="text-[11px] font-bold text-muted border border-cream-3 dark:border-white/10 rounded-md px-1.5 py-0.5">{s}</span>
                        ))}
                    </div>
                )}
    ...
    return (
        <div className={`min-h-screen bg-cream dark:bg-ink transition-colors ${fontClass}`}>
            ...
            <nav className="sticky top-0 z-30 bg-white/95 dark:bg-ink/95 backdrop-blur-md border-b border-cream-3 dark:border-white/10 h-14 flex items-center justify-between px-4 shadow-sm gap-3">
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => setCartOpen(true)}
                        className="relative bg-ink text-white w-9 h-9 rounded-xl flex items-center justify-center hover:bg-accent transition-colors">
                        🛒
                        {count > 0 && (
                ...
                <div className="hidden sm:flex flex-1 items-center gap-2 border-b-2 border-transparent focus-within:border-accent px-1 py-1 max-w-md transition-colors">
                ...
                <a href="/" className="text-lg font-black text-ink dark:text-cream shrink-0">
                    look<span className="text-accent">.141</span>
                </a>
            </nav>
            ...
            <div className="sm:hidden sticky top-14 z-20 bg-white dark:bg-ink border-b border-cream-3 dark:border-white/10 h-14 flex items-center px-4">
                <div className="w-full flex items-center gap-2 border-b-2 border-transparent focus-within:border-accent px-1 py-1.5 transition-colors">
                    <span className="text-muted text-sm shrink-0">🔍</span>
                    <input value={search} onChange={e => setSearch(e.target.value)}
                        placeholder={t('searchPlaceholder')}
                        className="bg-transparent outline-none border-0 appearance-none text-sm text-ink dark:text-cream w-full" />
            ...
            <div className="sm:hidden sticky top-28 z-20 bg-white dark:bg-ink border-b border-cream-3 dark:border-white/10">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-6 px-4 py-6 max-w-6xl mx-auto">
                    {dict.trustBadges.map((b, i) => (
                        <div key={i} className="flex items-center gap-3">
                            ...
                            <div className="w-11 h-11 rounded-full bg-accent-pale dark:bg-accent/10 flex items-center justify-center text-xl shrink-0">{b.icon}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="py-10 px-4">
                <h2 className="text-2xl font-black text-ink dark:text-cream text-center mb-6">{t('categoriesLabel')}</h2>
            ...
            <footer className="bg-ink px-4 py-10 text-center">
                <p className="font-black text-accent text-xl mb-1">look.141</p>
                ...
                <p className="text-white/10 text-xs">{dict.footerCopy(new Date().getFullYear())}</p>
            </footer>
    ...
    return (
        <section className="bg-cream dark:bg-ink py-12 px-4">
            <div className="max-w-2xl mx-auto">
                <p className="text-accent text-xs font-bold tracking-widest uppercase text-center mb-2">{t('faqEyebrow')}</p>
